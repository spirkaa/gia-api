...
class EmployeeFilter(FilterWithHelper):
    ...
    org__name = django_filters.CharFilter(lookup_expr="icontains", label="Место работы")
...
class PlaceFilter(FilterWithHelper):
    ...
    name = django_filters.CharFilter(lookup_expr="icontains", label="Наименование ППЭ")
    addr = django_filters.CharFilter(lookup_expr="icontains", label="Адрес ППЭ")
...
class ExamFilter(FilterWithHelper):
    ...
    date = django_filters.ModelChoiceFilter(
        queryset=models.Date.objects.all(), label="Дата экзамена"
    )
    level = django_filters.ModelChoiceFilter(
        queryset=models.Level.objects.all(), label="Уровень"
    )
    place__code = django_filters.CharFilter(label="Код ППЭ")
    place__name = django_filters.CharFilter(
        lookup_expr="icontains", label="Наименование ППЭ"
    )
    place__addr = django_filters.CharFilter(lookup_expr="icontains", label="Адрес ППЭ")
    employee__name = django_filters.CharFilter(
        lookup_expr="icontains", label="ФИО сотрудника"
    )
    employee__org__name = django_filters.CharFilter(
        lookup_expr="icontains", label="Место работы"
    )
